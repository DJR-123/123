...
class evsImageH5Dataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        videos = [
            os.path.join(opt.dataroot, name)
            for name in os.listdir(os.path.join(opt.dataroot))
            if name.endswith(".h5")
        ]
        ...
        for video_name in videos:
            ...
            self.files += [
                samples[i : i + opt.video_length] for i in range(1, num_samples)
            ]
    ...
    def __getitem__(self, index):
        ...
        for file_idx in f_index[:1]:
            ...
            if self.isTrain:
                ldr, ldr_params = ldr_generator(
                    img, self.under_over_ratio, augmentation=True, **ldr_params
                )
            else:
            ...
            if self.event_norm:
                ...
                mean, stddev = (
                    event_representation[event_representation != 0].mean(),
                    event_representation[event_representation != 0].std(),
                )
            ...
            if self.isTrain:
                ...
                img, event_representation, ldr = (
                    img.permute(2, 0, 1),
                    event_representation.permute(2, 0, 1),
                    ldr.permute(2, 0, 1),
                )
            else:
                img = tF.interpolate(img.unsqueeze(0), (self.height, self.width))[0]
                event_representation = tF.interpolate(
                    event_representation.unsqueeze(0), (self.height, self.width)
                )[0]
