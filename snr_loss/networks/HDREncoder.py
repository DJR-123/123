...
class UEncoder4Recurrent(nn.Module):
    def __init__(self, in_channels=3):
        ...
        self.Maxpool1 = RecurrentConvLayer(
            filters[0],
            filters[0],
            kernel_size=5,
            padding=2,
        ...
        self.Maxpool2 = RecurrentConvLayer(
            filters[1],
            filters[1],
            kernel_size=5,
            padding=2,
        ...
        self.Maxpool3 = RecurrentConvLayer(
            filters[2],
            filters[2],
            kernel_size=5,
            padding=2,
...
class UDecoder4(nn.Module):
    def __init__(self, out_channels=3):
        ...
        self.Up4 = UpsampleConvLayer(
            filters[3], filters[2], kernel_size=5, padding=2, norm="IN"
        )
        self.Up_conv4 = Conv2DBlock(filters[3], filters[2], norm="IN")

        self.Up3 = UpsampleConvLayer(
            filters[2], filters[1], kernel_size=5, padding=2, norm="IN"
        )
        self.Up_conv3 = Conv2DBlock(filters[2], filters[1], norm="IN")

        self.Up2 = UpsampleConvLayer(
            filters[1], filters[0], kernel_size=5, padding=2, norm="IN"
        )
...
class FusionLayer_SFT(nn.Module):
    def __init__(self, n_features):
        ...
        for i in range(n_features):
            self.weight.append(
                WeightBlock(in_channels=filters[i] * 2, out_channels=filters[i])
            )

    def forward(self, fe, fi):
        if self.n_features != len(fe) or self.n_features != len(fi):
            print(
                "The number of features does not match the input features of FusionLayer"
            )
        out = []
...
class HDRev_Encoder(nn.Module):
    def __init__(
        ...
        self.netEncoder_I.load_state_dict(
            torch.load(pretrained_I_path, weights_only=True)
        )

        self.netEncoder_E = UEncoder4Recurrent(in_channels=num_bins)
        self.netEncoder_E.load_state_dict(
            torch.load(pretrained_E_path, weights_only=True)
        )

        self.netMerge = FusionLayer_Unet(n_features=4)
        self.netMerge.load_state_dict(
            torch.load(pretrained_merge_path, weights_only=True)
        )

        self.netDecoder = UDecoder4(out_channels=3)
        self.netDecoder.load_state_dict(
            torch.load(pretrained_D_path, weights_only=True)
        )
    ...
    def forward(self, ldr, evs, return_img=False):
        ...
        for f in feat:
            ...
            # 3. 使用 tanh 归一化到 [-1, 1]，除以10使输入到tanh的值更合理
            f = torch.tanh(f / 10.0)
        ...
        if return_img:
            return feat_safe[-1], feat_safe, torch.flip(self.netDecoder(feat), dims=[1])
