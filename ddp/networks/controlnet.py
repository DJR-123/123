...
class ControlNetConditioningListEmbedding(nn.Module):
    """
    Quoting from https://arxiv.org/abs/2302.05543: "Stable Diffusion uses a pre-processing method similar to VQ-GAN
    [11] to convert the entire dataset of 512 × 512 images into smaller 64 × 64 “latent images” for stabilized
    training. This requires ControlNets to convert image-based conditions to 64 × 64 feature space to match the
    ...
    def __init__(
        self,
        conditioning_embedding_channels: int,
        conditioning_channels: Tuple[int, ...] = (64, 128, 256, 512),
        block_out_channels: Tuple[int, ...] = (16, 32, 96, 256),
        downsample=True,
    ):
        super().__init__()
        ...
        self.conv_in = nn.Conv2d(
            conditioning_channels[0], block_out_channels[0], kernel_size=3, padding=1
        )

        self.blocks = nn.ModuleList([])

        for i in range(len(block_out_channels) - 1):
            channel_in = block_out_channels[i] + conditioning_channels[i]
            channel_out = block_out_channels[i + 1]
            self.blocks.append(
                nn.Sequential(
                    nn.Conv2d(channel_in, channel_in, kernel_size=3, padding=1),
        ...
        self.conv_out = zero_module(
            nn.Conv2d(
                block_out_channels[-1] + conditioning_channels[-1],
                conditioning_embedding_channels,
                kernel_size=3,
                padding=1,
            )
    ...
    def forward(self, conditioning):
        embedding = self.conv_in(conditioning[0])
        embedding = F.silu(embedding)
        embedding_list = [embedding]
        for block, condition in zip(self.blocks, conditioning):
            # print(embedding.shape, condition.shape)
            embedding = block(torch.cat([embedding, condition], dim=1))
            embedding_list.append(embedding)

        embedding = self.conv_out(torch.cat([embedding, conditioning[-1]], dim=1))

        return embedding  # , embedding_list


class OursControlNetModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    """
    A ControlNet model.
    ...
    def __init__(
        ...
        self.controlnet_cond_embedding = ControlNetConditioningListEmbedding(
    ...
    def forward(
        ...
        controlnet_cond_ = self.controlnet_cond_embedding(controlnet_cond)
        # controlnet_cond_, controlnet_cond_list = self.controlnet_cond_embedding(controlnet_cond)
        sample = sample + controlnet_cond_
