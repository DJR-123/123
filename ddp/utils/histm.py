...
def local_int_align_satDark_hist_norm_pytorch(indata, ref, args=predefined_args()):
    ...
    indata_ds = F.interpolate(
        indata, scale_factor=1 / ds, mode="bilinear", align_corners=False
    )
    ref_ds = F.interpolate(
        ref, scale_factor=1 / ds, mode="bilinear", align_corners=False
    )

    meanMax_blend = args.meanMax_blend
    in_h, in_w = indata_ds.shape[-2], indata_ds.shape[-1]

    align_in_anchor = torch.zeros(
        (c, 6, patch_h * 2 - 1, patch_w * 2 - 1), device=indata.device
    )
    align_ref_anchor = torch.zeros(
        (c, 6, patch_h * 2 - 1, patch_w * 2 - 1), device=indata.device
    )
    ...
    for ci in range(c):
        for h in range(patch_h * 2 - 1):
            ...
            for w in range(patch_w * 2 - 1):
                ...
                patch_ref_nosat_anchor_conf = torch.clamp(
                    patch_ref_nosat_anchor_conf, 0, 1
                )

                a = 1 / (anchor_dark1 - anchor_dark0)
                b = anchor_dark0 / (anchor_dark0 - anchor_dark1)
                patch_ref_nodark_anchor_conf = a * patch_ref_anchor + b
                patch_ref_nodark_anchor_conf = torch.clamp(
                    patch_ref_nodark_anchor_conf, 0, 1
                )

                anchor_conf = (
                    patch_ref_nosat_anchor_conf + patch_ref_nodark_anchor_conf - 1
                )
                patch_target_anchor = (
                    patch_ref_anchor * anchor_conf + patch_in_anchor * (1 - anchor_conf)
                )
                ...
                norm_bot, norm_top = (
                    min(dark_min, patch_target_anchor[0]),
                    patch_target_anchor[as_anchor_idx],
                )
                norm_anchor = (
                    patch_target_anchor[: as_anchor_idx + 1] - norm_bot + eps
                ) / (norm_top - norm_bot + eps) * (norm_top - dark_min) + dark_min
                patch_target_anchor[: as_anchor_idx + 1] = norm_anchor

                norm_bot, norm_top = patch_target_anchor[ds_anchor_idx], max(
                    1, patch_target_anchor[-1]
                )
                norm_anchor = (
                    patch_target_anchor[ds_anchor_idx + 1 :] - norm_bot + eps
                ) / (norm_top - norm_bot + eps) * (1 - norm_bot) + norm_bot
    ...
    kernel = torch.tensor(
        np.dot(filter, np.transpose(filter)), device=indata.device, dtype=torch.float32
    )
    kernel_size = kernel.shape[-1]

    c, _, h_before, w_before = align_in_anchor.shape
    align_in_anchor = F.pad(
        align_in_anchor,
        (kernel_size // 2, kernel_size // 2, kernel_size // 2, kernel_size // 2),
        mode="replicate",
    )
    align_ref_anchor = F.pad(
        align_ref_anchor,
        (kernel_size // 2, kernel_size // 2, kernel_size // 2, kernel_size // 2),
        mode="replicate",
    )
    ...
    align_in_anchor = F.conv2d(
        align_in_anchor.reshape(c * 6, 1, h, w),
        kernel.unsqueeze(0).unsqueeze(0),
        padding="valid",
    ).reshape(c, 6, h_before, w_before)
    align_ref_anchor = F.conv2d(
        align_ref_anchor.reshape(c * 6, 1, h, w),
        kernel.unsqueeze(0).unsqueeze(0),
        padding="valid",
    ).reshape(c, 6, h_before, w_before)

    align_in_anchor = F.interpolate(
        align_in_anchor, size=[ori_h, ori_w], mode="bilinear", align_corners=False
    )
    align_ref_anchor = F.interpolate(
        align_ref_anchor, size=[ori_h, ori_w], mode="bilinear", align_corners=False
    )
...
def local_int_align_satDark_hist_norm(indata, ref, args=predefined_args()):
    ...
    for h in range(patch_h * 2 - 1):
        ...
        for w in range(patch_w * 2 - 1):
            ...
            patch_target_anchor = patch_ref_anchor * anchor_conf + patch_in_anchor * (
                1 - anchor_conf
            )
            ...
            norm_anchor = (
                patch_target_anchor[: as_anchor_idx + 1] - norm_bot + 1e-10
            ) / (norm_top - norm_bot + 1e-10) * (norm_top - dark_min) + dark_min
            ...
            norm_anchor = (
                patch_target_anchor[ds_anchor_idx + 1 :] - norm_bot + 1e-10
            ) / (norm_top - norm_bot + 1e-10) * (1 - norm_bot) + norm_bot
            patch_target_anchor[ds_anchor_idx + 1 :] = (
                norm_anchor  # warning, in C++ [idx+1] may exceed boundary
            )
    ...
    align_in_anchor = cv2.filter2D(
        align_in_anchor, -1, kernel, borderType=cv2.BORDER_REPLICATE
    )
    align_ref_anchor = cv2.filter2D(
        align_ref_anchor, -1, kernel, borderType=cv2.BORDER_REPLICATE
    )
...
def local_int_align_satDark_hist_norm_sat(indata, ref, args):
    ...
    for h in range(patch_h * 2 - 1):
        ...
        for w in range(patch_w * 2 - 1):
            ...
            patch_conf = (
                np.sum(patch_ref_nosat.flatten() * patch_ref_nodark.flatten()) / tmplen
            )
            ...
            anchor_conf = patch_ref_nosat_anchor_conf + patch_ref_nodark_anchor_conf - 1
            anchor_conf = patch_conf * anchor_conf
            patch_target_anchor = patch_ref_anchor * patch_conf + patch_in_anchor * (
                1 - patch_conf
            )
...
def local_int_align_satDark_hist_norm_RGB(src_rgb, dst_rgb):
    ...
    for i in range(b):
        ...
        aligned_rgb[i : i + 1] = local_int_align_satDark_hist_norm_pytorch(
            src_rgb[i : i + 1], dst_rgb[i : i + 1], args
        )
...
if __name__ == "__main__":
    ...
    cv2.imwrite(
        "./aligned_rgb.png",
        np.concatenate([src_rgb_org, dst_rgb_org, aligned_rgb * 255], axis=-2)[
            :, :, ::-1
        ],
    )
