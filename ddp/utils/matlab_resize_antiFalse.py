...
def matlab_resize(
    img, scale=None, output_size=None, interploate="bilinear", antialiasing=False
):
...
def interploate_impl(adj4_cor, adj4_val, src_cor, c):
    ...
    Q = (
        (1 - A - B + A * B) * adj4_val[0 : 1 * c]
        + (A - A * B) * adj4_val[1 * c : 2 * c]
        + (B - A * B) * adj4_val[2 * c : 3 * c]
        + A * B * adj4_val[3 * c :]
    )
...
if __name__ == "__main__":
    ...
    img = cv2.imread(
        "/home/SENSETIME/weiyunxuan1/Downloads/sr_052647/sr_052647_02_ev0_rs.png"
    )
    img_rs = matlab_resize(img, output_size=[173, 225])
    cv2.imwrite(
        "/home/SENSETIME/weiyunxuan1/Downloads/sr_052647/sr_052647_02_ev0_rs_0p5_test_python.png",
        img_rs,
    )
