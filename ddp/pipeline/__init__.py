...
def find_pipeline_using_name(pipeline_type):
    ...
    for name, cls in pipelinelib.__dict__.items():
        if name.lower() == target_pipeline_name.lower() and issubclass(
            cls, DiffusionPipeline
        ):
            pipeline = cls
    if pipeline is None:
        raise NotImplementedError(
            "In %s.py, there should be a subclass of DiffusionPipeline with class name that matches %s in lowercase."
            % (pipeline_filename, target_pipeline_name)
        )
