...
class Stagepipeline(DiffusionPipeline):
    ...
    def prepare_latents(
        self, batch_size, num_channels, height, width, device, generators, latents=None
    ):
        shape = (
            batch_size,
            num_channels,
        ...
        if isinstance(generators, list) and len(generators) != batch_size:
            raise ValueError(
                f"the length of generators {len(generators)} is not capable of batch size {batch_size}"
            )
        if latents is None:
            if isinstance(generators, list):
                latents = [
                    torch.randn(shape, generator=generators[i], device=device)
            ...
            if latents.shape != shape:
                raise ValueError(
                    f"the shape of the init latents ({latents.shape}) is not matched with the expected latent shape {shape}"
                )
            latents = latents.to(device)
        return latents * self.scheduler.init_noise_sigma, latents

    def prepare_extra_step_kwargs(self, generator, eta):
        accept_eta = "eta" in set(
            inspect.signature(self.scheduler.step).parameters.keys()
        )
        accept_generator = "generator" in set(
            inspect.signature(self.scheduler.step).parameters.keys()
        )
    ...
    def __call__(
        ...
        if guidance_scale < 1:
            raise ValueError(
                f"guidance scale should greater than one, while the input is {guidance_scale}"
            )
        ...
        conditions, condition_list = self.cond_encoder(
            condition_images, condition_events
        )
        ...
        if latents is None:
            latents, noise = self.prepare_latents(
                batch_size, num_channels_latents, height, width, device, generators
            )
        ...
        for i, t in enumerate(timestamps):
            ...
            if not self.isVal:
                down_block_res_samples = [
                    torch.cat([torch.zeros_like(d), d], dim=0)
                    for d in down_block_res_samples
                ]
                mid_block_res_samples = torch.cat(
                    [torch.zeros_like(mid_block_res_samples), mid_block_res_samples],
                    dim=0,
                )
        ...
        if return_latents:
            ...
            if self.upsampler_w != None:
                results = self.upsampler_w.decode(
                    latents / self.upsampler_w.config.scaling_factor, condition_list
                ).sample
            else:
                results = None
